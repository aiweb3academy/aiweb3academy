...
function List({ title, items }: { title: string; items: { title: string; url: string }[] }) {
  return (
      ...
      <div className="flex flex-wrap justify-between gap-10">
        ...
        <div className="flex flex-col gap-4 lg:items-end">
          <ul className="flex gap-5">
            {[
              { url: 'https://github.com/aiweb3academy/aiweb3academy', icon: FaGithub },
              { url: 'https://discord.aiweb3.academy', icon: FaDiscord },
              { url: 'https://x.com/aiweb3academy', icon: FaXTwitter }
            ].map(({ url, icon: Icon }) => (
              <li key={url}>
                <a href={url} target="_blank" rel="noreferrer" className="block transition-colors hover:text-primary">
          ...
          >
            Powered by{' '}
            <svg viewBox="0 0 361 70" fill="currentColor" className="h-3 shrink-0">
              <path
