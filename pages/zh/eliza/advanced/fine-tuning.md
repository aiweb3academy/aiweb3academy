...
async function embed(runtime: IAgentRuntime, input: string): Promise<number[]> {
  ...
  // 生成新的嵌入
  const response = await runtime.fetch(`${runtime.modelProvider.endpoint}/embeddings`, {
    method: 'POST',
    headers: {
...
```typescript
async function selectOptimalModel(task: string, requirements: ModelRequirements): Promise<ModelClass> {
...
```typescript
async function validateEmbedding(embedding: number[], expectedDimensions: number = 1536): Promise<boolean> {
